...
function json(res, code, obj) {
  ...
  if (code >= 400) {
    ...
    if (obj) {
      if (typeof obj.error_code === 'number') {
        ...
        if (typeof obj.error === 'number') {
          ...
          if (obj.error === 'TOKEN_DANG_BAN' || obj.error === 'TOKEN_BUSY') errorCode = 423;
          else if (obj.error === 'THIEU_SERIAL') errorCode = 4001;
          else if (obj.error === 'KHONG_CO_USB_TOKEN') errorCode = 5031;
          else {
            const parsed = parseInt(obj.error, 10);
            errorCode = isNaN(parsed) ? -1 : parsed;
          }
        }
      }
    ...
    let data = null;
    if (obj) {
      const { error, error_code, message: msg, ...extra } = obj;
